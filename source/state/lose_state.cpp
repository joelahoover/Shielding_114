...
LoseState::LoseState(std::unique_ptr<State> game_state) :
	_message_texture(),
	_back_texture(),
	_message_sprite(),
	_back_sprite(),
...
{
	// Load the textures
	_message_texture.loadFromFile("resources/graphics/objective_failed.png");
	...
	_dimmer.setFillColor({0, 0, 0, 192});
}
...
void LoseState::update(std::unique_ptr<State>& current_state) {
	if(_next_state) {
		current_state.reset(_next_state.release());
	}
}

void LoseState::draw(sf::RenderTarget& target, sf::RenderStates states) const {
